...
# Skriv eit program for å rekne ut og skrive ut summen av kvadrata til tosifra tal frå lista numbers,
# som er delelege med 7 utan rest.


def is_to_digits_number(number):
    return 9 < abs(number) < 100 and number % 7 == 0


def filter(func, iter_obj):
    result = []
    for item in iter_obj:
        if is_to_digits_number(item):
            result.append(item)
    return result
...
if __name__ == "__main__":
    ...
    result = map(lambda x: x**2, filter(is_to_digits_number, numbers))
    print(sum(result))
