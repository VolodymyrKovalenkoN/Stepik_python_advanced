...
# в котором для каждого владельца будут перечислены его собаки. Ключом словаря должен быть кортеж 
# (имя, фамилия, возраст владельца), а значением – список кличек собак (сохранив исходный порядок следования).
...
for pet, name, last_name, age in pets:
    owner = (name, last_name, age)

    if owner not in result:
        result[owner] = [pet,]
    else:
        result[owner].append(pet)
print(result)
